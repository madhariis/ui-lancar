<template>
    <div class="bill-wrapper">
        <div class="bill-header">
            <router-link tag="button" class="left" to="/detail"><i class="fa fa-arrow-left"></i></router-link>
            <div>Billing</div>
            <div class="right"></div>
        </div>
        <div class="billinginfo">
            <img src="http://via.placeholder.com/50x50" alt="">
            <div class="billinginfo-header">
                <h4>Billing Info</h4>
            </div>
            <div class="billinginfo-body">
                <div v-for="(list,index) in items" :key="index" :class="'billinginfo-body-text'">
                    <div>{{list.menu.title}}</div>
                    <div class="num">{{list.qty}}</div>
                    <div class="price">Rp. {{list.menu.price}}</div>
                </div>
                <div class="line"></div>
                <div :class="'billinginfo-body-bot'">
                    ...
                    <div>Total</div>
                    <div class="prc">Rp. {{calculated.total}}</div>
                </div>
                <div :class="'billinginfo-body-bot'">
                    <div style="width: 20px">
                        <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                    ...
                    <div>Discount</div>
                    <div class="prc">Rp. {{calculated.discount}}</div>
                </div>
                <div :class="'billinginfo-body-bot'">
                    <div style="width: 20px">
                        <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                    ...
                    <div>Tax</div>
                    <div class="prc">Rp. {{calculated.tax}}</div>
                </div>
            </div>
            <div class="billinginfo-footer">
                <div>Grand Total</div>
                <div class="foot">Rp. {{grandTotal}}</div>
            </div>
           <div class="billinginfo-btn">
             <button @click="paid()" class="secondary">End Process</button>
           </div>
        </div>
    </div>
</template>
...
export default {
  ...
  methods: {
    getData() {
      (async () => {
        let data = await order.getMyTransaction({
          token: this.token,
    ...
    paid() {
      (async () => {
        let data = await order.closeMyTransaction({
          token: this.token,
          tableId: this.tableId,
          transactionId: this.transactionId
        })
  ...
  computed: {
    ...
    calculated: function () {
      if (this.transaction) {
        return {
          ...
          tax: (this.transaction.total - this.transaction.discount) * (10/100)
        }
      } else {
        return {
          total: 0,
    ...
    grandTotal: function () {
      return this.calculated.total - this.calculated.discount + this.calculated.tax
    }
  }
}
</script>
